        ...
        try:
            ...
            if not created:
                return JsonResponse(
                  ...
                  else redirect('/applications/status/')

            # ---- Save form data ----
            app.qualification = request.POST.get('qualification', '')
            app.experience = int(request.POST.get('experience', 0))
            ...
            return JsonResponse(
                {'success': True, 'application_id': app.id},
                status=201
            ) if request.headers.get('Accept') == 'application/json' \
              else redirect('/applications/status/')
...
def allcomponies(request):
    return render(request, 'applications/allcomponies.html')

def topcomponies(request):
    return render(request, 'applications/topcomponies.html')

def hiring(request):
    return render(request, 'applications/hiring.html')